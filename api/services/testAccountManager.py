...
from accountManager import (is_valid_email, check_email_in_use,
                            validate_request, extract_and_prepare_attributes,
                            check_for_extra_attributes, check_required_attributes,
                            get_account_info, validate_get_request,
                            extract_and_prepare_attributes_for_get,
                            validate_queried_attributes, create_account,
                            validate_create_request, update_account,
                            validate_update_request, delete_account,
...
class TestValidateRequest(unittest.TestCase):
    ...
    def test_invalid_email_in_request(self):
        ...
        self.assertEqual(validate_request(request),
                         (False, "Invalid or missing email identifier."))
    ...
    def test_invalid_account_type(self):
        ...
        self.assertEqual(validate_request(request),
                         (False, "Invalid object type. Must be one of "
...
class TestExtractAndPrepareAttributes(unittest.TestCase):
    ...
    def test_with_missing_object_type(self):
        request = {
            'attributes': {'title': 'Concert', 'date': '2024-01-01'}
        }
...
class TestCheckForExtraAttributes(unittest.TestCase):
    ...
    def test_no_extra_attributes(self):
        ...
        object_type = 'artist'
        self.assertTrue(check_for_extra_attributes(validation_attributes, object_type)[0])

    @patch('accountManager.attributes_schema', attributes_schema)
    def test_with_extra_attributes(self):
        validation_attributes = {'name': 'John Doe', 'genre': 'Rock', 'extra': 'Not Allowed'}
        object_type = 'artist'
        self.assertFalse(check_for_extra_attributes(validation_attributes, object_type)[0])
...
class TestCheckRequiredAttributes(unittest.TestCase):
    ...
    def test_all_required_attributes_provided(self):
        ...
        object_type = 'artist'
        self.assertTrue(check_required_attributes(validation_attributes, object_type)[0])

    @patch('accountManager.attributes_schema', attributes_schema)
    def test_missing_required_attributes(self):
        validation_attributes = {'name': 'John Doe', 'genre': 'Rock'}  # Missing 'country'
        object_type = 'artist'
        self.assertFalse(check_required_attributes(validation_attributes, object_type)[0])
...
class TestValidateGetRequest(unittest.TestCase):
    ...
    @patch('accountManager.extract_and_prepare_attributes_for_get')
    @patch('accountManager.validate_queried_attributes')
    def test_queried_attributes_validation_fails(self, mock_validate_queried, mock_extract):
...
class TestValidateQueriedAttributes(unittest.TestCase):
    ...
    def test_invalid_attribute_in_queried_attributes(self):
        ...
        object_type = 'artist'
        self.assertFalse(validate_queried_attributes(queried_attributes, object_type)[0])
...
class TestCreateAccount(unittest.TestCase):
    ...
    def test_exception_during_insert(self, mock_supabase, mock_validate):
        mock_validate.return_value = (True, "")
        mock_supabase.table().insert().execute.side_effect = Exception("Database connection error")
...
class TestValidateCreateRequest(unittest.TestCase):
    ...
    def test_successful_validation(self, mock_check_extra, mock_check_required, mock_extract):
    ...
    def test_extra_undefined_attributes(self, mock_check_extra, mock_extract):
        ...
        mock_extract.return_value = ('artist', {'email': 'user@example.com', 'username': 'John',
                                                'genre': '', 'extra_attr': 'Not Allowed'})
        mock_check_extra.return_value = (False, "Additional, undefined attributes cannot be specified.")
        ...
        self.assertFalse(valid)
        self.assertEqual(message, "Additional, undefined attributes cannot be specified.")

    @patch('accountManager.extract_and_prepare_attributes')
    def test_attributes_without_value(self, mock_extract):
        # Simulate an attribute without a value
        mock_extract.return_value = ('artist', {'email': 'user@example.com', 'username': 'John', 'genre': ''})

        request = {
            'object_type': 'artist',
            'attributes': {'email': 'user@example.com', 'username': 'John', 'genre': ''}
        }
        valid, message = validate_create_request(request)
...
class TestValidateUpdateRequest(unittest.TestCase):
    ...
    def test_valid_update_request(self, mock_check_extra, mock_extract):
        ...
        request = {
            'object_type': 'artist',
            'attributes': {'name': 'New Artist Name'}
        }
    ...
    def test_no_attributes_specified_for_update(self, mock_extract):
        ...
        self.assertFalse(valid)
        self.assertEqual(message, "At least one attribute must be specified for update.")
    ...
    def test_extra_undefined_attributes_provided(self, mock_check_extra, mock_extract):
        ...
        request = {
            'object_type': 'artist',
            'attributes': {'undefined_attr': 'value'}
        }
        valid, message = validate_update_request(request)
    ...
    @patch('accountManager.extract_and_prepare_attributes')
    @patch('accountManager.check_for_extra_attributes')
    def test_valid_request_with_multiple_attributes(self, mock_check_extra, mock_extract):
        # Setup for a valid request with multiple attributes
        mock_extract.return_value = ('venue', {'location': 'New Location', 'capacity': 5000})
        mock_check_extra.return_value = (True, "")
...
class TestDeleteAccount(unittest.TestCase):
    ...
    def test_exception_during_delete_operation(self, mock_supabase, mock_validate):
        mock_validate.return_value = (True, "")
        mock_supabase.table().delete().eq().execute.side_effect = Exception("Database error")
